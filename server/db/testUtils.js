...
const deleteTables = (db) => {
  return db.none(`DROP TABLE IF EXISTS users, questions, answers, votes, categories CASCADE;`)
}
...
const seedTestData = (db) => {
  return db.none('INSERT INTO users (username, email, img_url, bio) VALUES ($1, $2, $3, $4)', ['JohnSmith', 'js@example.com', 'www.example.com/avatar.png', 'My first profile'])
...
const seedDummyData = (db) => {
  ...
  .then(() => db.none(`INSERT INTO questions (user_id, message, coordinates, location, category_id, active)
    VALUES
  ...
  .then(() => db.none(`INSERT INTO answers (user_id, question_id, message)
    VALUES
  ...
  .then(() => db.none(`INSERT INTO categories (name)
    VALUES
    ('chipotle'),
    ('convention'),
    ('sports'),
    ...
    ('love'),
    ('politics')`))
  .then(() => db.none(`INSERT INTO votes (user_id, answer_id, vote_type)
    VALUES (1, 1, 1)`))
  .catch((error) => {
    console.log('Seeding dummy data caused an error:', error)
    throw error
  })
}
